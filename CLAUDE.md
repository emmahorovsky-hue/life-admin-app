...
```bash
cd server && NODE_ENV=test DISABLE_AUTH_RATE_LIMIT=true DISABLE_RATE_LIMIT=true npm run dev
```

`NODE_ENV=test` is what the CI e2e job sets, and it is there for the same reason: `services/emailService.ts` refuses to construct a Resend client under it, so registration doesn't await a live HTTPS call to api.resend.com. `npm run dev` alone leaves `NODE_ENV` unset, so a developer with a real `RESEND_API_KEY` in `server/.env` reproduces the CI flake locally *and* mails ~20 `user<ts>@example.com` addresses per run — RFC 2606 guarantees those bounce, and the bounces count against the domain real verification mail leaves from. It also makes both rate-limit flags belt-and-braces rather than load-bearing (see below), and switches off CSRF validation (`middleware/csrf.ts`), which is the one thing a local run stops exercising — matching CI, which never exercised it either. If you specifically want CSRF live, drop `NODE_ENV=test` and use `DISABLE_EMAIL_SENDING=true` instead: same email guard, development everywhere else.

Without that flag the last two auth specs (`Session Persistence › user stays logged in after page refresh`, `Logout › user can logout`) fail on a clean checkout. Each test registers a fresh user, and by the end of the run the auth limiter rejects `/api/auth/register`; the tests surface it as "expected `/dashboard`, got `/register`", which reads exactly like an auth regression but is ordering-dependent — the same tests pass in isolation. **Before debugging an e2e auth failure *locally*, grep the backend log for `auth.rate_limit.exceeded`.** The flag is safe to keep in `server/.env`: `routes/auth.ts` ignores it when `NODE_ENV=production`.

**None of this applies to a failure in CI.** The e2e job sets `NODE_ENV: test` at job level, which makes `skipAuthRateLimit` true and the general limiter's `enabled` false — both limiters are inert, so that grep is always empty there and the rate-limit trail is a dead end. "Expected `/dashboard`, got `/register`" in CI has a different cause: registration awaits the verification email, and CI's dummy `RESEND_API_KEY` used to be enough to make that a live call to api.resend.com inside the request (~0.5s → ~2.5s), which blew Playwright's 5s assertion timeout on a different test each run. Fixed in `services/emailService.ts` by refusing to construct a client under `NODE_ENV=test` (or `DISABLE_EMAIL_SENDING=true` outside production), with `retries` on CI as a backstop. **A dependabot PR with a red E2E is almost always this, not the bump — re-run before believing it.**
...
| `DISABLE_EMAIL_SENDING` | Dev only: `true` makes `services/emailService.ts` skip constructing the Resend client even with a key set, so every send logs to stdout instead. Implied by `NODE_ENV=test`; set it explicitly when you want a development-mode server that still mails nobody (e.g. a local e2e backend with CSRF left on). Ignored (with a warning) when `NODE_ENV=production`, so a leaked flag can't silently stop verification and password-reset mail |
...
| `ENABLE_PUSH_REMINDERS` | Master switch for the renewal-reminder **push** channel (`services/renewalReminderService.ts`), separate from the per-user `reminderPushEnabled` toggle. Default **off** — the channel ships dark. It exists because `reminderPushEnabled` defaults to `true` and device tokens have been collected since LIF-115, so turning the send path on notifies every existing user immediately, from whatever app build they have installed; App Store review means the server deploy and the build carrying the in-app toggle cannot ship together. Set to exactly `"true"` once that build is live. Read per call, so flipping it needs no restart (LIF-250) |
| `EXPO_ACCESS_TOKEN` | Authenticates renewal-reminder pushes to Expo (`services/pushService.ts`). Optional **today** — sending works without it — but it becomes required the moment "enhanced push security" is switched on for the Expo project, and until it is set, anyone holding a leaked device token can notify our users. The push channel wants **Node 22.12+** (we pin 24): `expo-server-sdk` v6 is pure ESM and older runtimes cannot `require()` it. `pushService` loads the SDK on first send, not at import, so an older runtime costs you the push channel — not the process (LIF-250) |
