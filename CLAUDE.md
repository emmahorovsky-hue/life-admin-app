...
| `API_RATE_LIMIT_MAX` / `API_RATE_LIMIT_WINDOW_MS` | General limiter ceiling and window (defaults 1000 / 15 min, per authenticated user per route group; per IP when there's no valid token) |
